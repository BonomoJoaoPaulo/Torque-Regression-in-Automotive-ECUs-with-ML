...
print(f"RMSE: {rmse_svr}, MAE: {mae_svr}, R²: {r2_svr}")

# Plot actual vs predicted
plt.figure(figsize=(14, 7))
plt.plot(y_test.index, y_test, label='Actual', color='blue')
plt.plot(y_test.index, predictions_svr, label='Predicted', linestyle='--', color='red')
plt.title('Actual vs Predicted Torque using LinearSVR')
plt.xlabel('Index')
plt.ylabel('Torque (N.m)')
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(PLOT_DIR, 'comparison_over_time_svr.png'), dpi=300)
plt.show()
...
plt.title('Feature Coefficients in LinearSVR')
plt.xlabel('Coefficient Value')
plt.ylabel('Features')
plt.tight_layout()
plt.savefig(os.path.join(PLOT_DIR, 'feature_coefficients_svr.png'), dpi=300)
plt.show()

# Save the model
joblib_file = os.path.join(MODEL_DIR, 'linear_svr_model.joblib')
